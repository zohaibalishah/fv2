...
const {DB_Name, DB_Password, DB_Username, DB_Url, DB_Host, DB_Port} = process.env;
...
var connect = mongoose.connect(`${DB_Url}://${DB_Username}:${DB_Password}@${DB_Host}:${DB_Port}/${DB_Name}`, {
...
.then(() => {
console.log("Connection established with" + " " + `${DB_Url}://${DB_Username}:${DB_Password}@${DB_Host}:${DB_Port}/${DB_Name}`);
